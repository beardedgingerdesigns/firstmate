...
# (the branch a task worktree is cut from), bin/fm-brief.sh (the PR base a
# worker is told), and bin/fm-promote.sh. base=<branch> is orthogonal to every
# other token: it names the project's work branch, the one task branches are cut
# from and PRs target, when that differs from the forge's default branch. An
# empty base= or one that is not a valid branch name exits 1 rather than
...
# `branch`, or `base` resolves as it did before the forge existed, and in the mode slot it
...
NAME=${1:?usage: fm-project-mode.sh [--raw|--branch-prefix|--forge|--base] <project-name>}
...
parsed=$(awk -v n="$NAME" '
  ...
  $1=="-" && $2==n {
    ...
    if ($3 ~ /^\[/) {
      ...
      # base=<branch> (read by --base above) are recognized by their own shape
      # wherever they appear, other keyed tokens are ignored (with a near-miss warning for the forge
