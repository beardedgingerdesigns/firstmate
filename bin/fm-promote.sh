...
# immutable ship branch, and - rendered from
...
# up. The registry IS read for two project facts rather than per-task decisions,
# so promotion takes them from there instead of asking firstmate to remember
# them: the project's forge binding, and its registered work branch (base=) as
# the clean and PR base.
...
PR_BASE=
if [ -n "$PROMOTE_PROJECT" ]; then
  PR_BASE=$(FM_HOME="$FM_HOME" FM_DATA_OVERRIDE="$DATA" "$FM_ROOT/bin/fm-project-mode.sh" --base "$PROMOTE_PROJECT_NAME") || exit 1
fi
...
3. Return to $CLEAN_BASE, then create your branch: \`git checkout -b $BRANCH_Q --\`.
...
EOF
  ...
  printf '\n'
  fm_dod_block "$MODE" "$ID" "$BRANCH" "$FORGE" "$PR_BASE"
}
mkdir -p "$DATA/$ID"
