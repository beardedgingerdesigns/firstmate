...
- gerritproj [direct-PR forge=gerrit base=dev] - fixture (added 2026-09-25)
EOF
  ...
  # A Gerrit project has no pull request, so its work branch is the change target.
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-base-gerrit gerritproj --mode direct-PR --forge gerrit >/dev/null 2>&1 \
    || fail "gerrit brief on a registered base should scaffold"
  assert_grep 'gerrit-axi publish --squash --json --branch dev' "$home/data/brief-base-gerrit/brief.md" \
    "gerrit brief did not name the registered work branch as the change target"
  assert_no_grep 'pr create --base' "$home/data/brief-base-gerrit/brief.md" \
    "gerrit brief was told to open a pull request"
